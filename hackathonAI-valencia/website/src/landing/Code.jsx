...
function CodePanel({ language, tag, label, code, children }) {
  ...
  return (
    <div className="group dark:bg-white/2.5">
      ...
      <div className="relative">        
        ...
        <CopyButton code={child.props.children ?? code} />
      </div>
    </div>
  )
}
            ...
            <Tab
              className={clsx(
                ...
                childIndex === selectedIndex
                  ? 'border-yellow-500 text-[#FEDE00]/100'
    ...
    return (
      <Tab.Panels>
        {Children.map(children, (child) => (
          <Tab.Panel>
            <CodePanel {...props} {...child.props}>{child}</CodePanel>
          </Tab.Panel>
        ))}
      </Tab.Panels>
    )
...
export function Code({ language, children }) {
  ...
  useEffect(() => {
    /*highlighter.then(highlighter => {
      const code = highlighter.codeToHtml(children, { lang: language })
      codeRef.current.innerHTML = code.replace("#2e3440ff", "");      
    })*/            
  }, [language, children]);

  return <div ref={codeRef}>{children}</div>;
};
